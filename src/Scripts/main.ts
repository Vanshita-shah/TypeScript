...
function memoryHandler(obj:HTMLButtonElement) {
  ...
  let MCBtn:HTMLButtonElement = parent?.firstElementChild?.firstElementChild as HTMLButtonElement;
  let MRBtn:HTMLButtonElement = parent?.firstElementChild?.nextElementSibling?.firstElementChild as HTMLButtonElement;
  let MSBtn:HTMLButtonElement = parent?.lastElementChild?.firstElementChild as HTMLButtonElement;
  ...
  switch (val) {
    ...
    case "M+":
      ...
      memoryDisplay.value = String(Number(memoryDisplay.value) + Number(input.value));
      memoryDisplay.parentElement?.classList.remove("invisible");
      MCBtn.disabled = false;
      MRBtn.disabled = false;
...
function getVal(exp:string):[string,string]|string|undefined {
  if (exp[exp.length - 1] === "!") {
    if(exp[exp.length - 2]===")"){
      return [exp.slice(exp.lastIndexOf("(")), String(evaluate(String(getVal(exp.slice(0,-1)))))];
    }
    let i = exp.length - 1;
